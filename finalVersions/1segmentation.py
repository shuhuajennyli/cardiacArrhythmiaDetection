...
for dirname, _, filenames in os.walk('/Users/jennyli/Documents/scienceFairDL/cardiacArrhythmiaDetection/FinalVersions/mitbih-database/mitbih_database'):
...
def process(wave, annotations):
	...
	#reshaping it into one row then turning into a dataframe
	...
	rhythm_change = []
	#starts at 1 because of the index (removed headings)
	for x in range(1, len(beat)+1):
	#+ shows when its a rhythm change
		if beat_type[x]== '+':
	...
	processed = open("/Users/jennyli/Documents/scienceFairDL/cardiacArrhythmiaDetection/finalVersions/processednew.csv", "a+")


	#adding 144 samplings to the front and back of the peak of the QRS complex assuming each beat is 0.8 seconds (80bpm)
	for x in range(1, len(beat)+1):
		#making sure it's an actual beat (get rid of things like "bad connection")
		if beat_type[x] == 'N' or beat_type[x] == 'Q' or beat_type[x] == 'x' or beat_type[x] == 'f' or beat_type[x] == '/' or beat_type[x] == '!' or beat_type[x] == 'E' or beat_type[x] == 'V' or beat_type[x] == 'F' or beat_type[x] == 'J' or beat_type[x] == 'S' or beat_type[x] == 'a' or beat_type[x] == 'L' or beat_type[x] == 'R' or beat_type[x] == 'A':
			...
			if rhythmannotations[x] =='(AFIB' or rhythmannotations[x] == '(VFL' or rhythmannotations[x] == '(N' or rhythmannotations[x] == '(AFL' or rhythmannotations[x] == '(BII':
			#making sure there is enough data to get the beat wave, or else there will be an additional annotation
				if beat[x] > 144 and beat[x] < 649856: 
					...
					for item in segment_list:
						#comma to make it in the form of a csv file
						processed.write("%s," % item)
				
					processed.write(segment_type)
					#enter to start the next beat
					processed.write("\n")
...
for x in range(96):
	#the first file of two is the wave file
	if x % 2 == 0:
