...
def network(X_train,y_train,X_validation,y_validation):
    ...
    callbacks = [ModelCheckpoint(filepath='seperatefile2_7501dense3.h5', monitor='val_loss', save_best_only=True)]

    #training the model and saving it to a file, and saving history for validation and loss graphs
    history=model.fit(X_train, y_train,epochs=750, callbacks=callbacks, batch_size=100,validation_data=(X_validation,y_validation))
    model.load_weights('seperatefile2_7501dense3.h5')
    model.save('seperatefile2_7501dense3.h5')
