...
df=pd.read_csv('/home/cst2020/ScienceFair/processedtopseven.csv',header=None)
...
df_4=(df[df[288]==4]).sample(n = 1359, random_state = 8)
df_3=(df[df[288]==3]).sample(n = 1381, random_state = 888)
df_6=(df[df[288]==6]).sample(n = 472, random_state = 888)
df_5=(df[df[288]==5]).sample(n = 422, random_state = 888)
...
df_6_train = df_6.sample(frac = 0.8, random_state = 888)
df_6_test = df_6.drop(df_6_train.index)

df_5_train = df_5.sample(frac = 0.8, random_state = 888)
df_5_test = df_5.drop(df_5_train.index)

#putting all the test into one dataframe
test_df = pd.concat([df_0_test, df_1_test, df_2_test, df_3_test, df_4_test, df_6_test, df_5_test])
...
df_6_atrain = df_6_train.sample(frac = 0.8, random_state = 42)
df_6_val = df_6_train.drop(df_6_atrain.index)

df_5_atrain = df_5_train.sample(frac = 0.8, random_state = 42)
df_5_val = df_5_train.drop(df_5_atrain.index)

#combining all the validation samples
validation_df = pd.concat([df_2_val, df_0_val, df_1_val, df_4_val, df_3_val, df_6_val, df_5_val])

#sampling and upsampling the beats for actual train
df_2_sample = (df_2_atrain.sample(n = 20000, random_state = 42))
df_0_upsample = resample(df_0_atrain, n_samples = 20000, random_state = 100)
df_1_upsample = resample(df_1_atrain, n_samples = 20000, random_state = 88)
...
df_5_upsample = resample(df_5_atrain, n_samples = 20000, random_state = 888)
df_6_upsample = resample(df_6_atrain, n_samples = 20000, random_state = 888)

#combining all the actual train samples
train_df = pd.concat([df_2_sample, df_0_upsample, df_1_upsample, df_3_upsample, df_4_upsample, df_5_upsample, df_6_upsample])
...
def network(X_train,y_train,X_validation,y_validation):
    ...
    conv2_1=Convolution1D(32, 3, activation='relu', input_shape=im_shape)(conv1_1)
    conv2_1=BatchNormalization()(conv2_1)
    conv2_1=Dropout(0.5)(conv2_1)
    conv2_1=Convolution1D(32, 3, activation='relu', input_shape=im_shape)(conv2_1)
    ...
    conv3_1=Convolution1D(32, 3, activation='relu', input_shape=im_shape)(conv3_1)
    ...
    main_output = Dense(7, activation='softmax', name='main_output')(flatten)
    ...
    history=model.fit(X_train, y_train,epochs=100,callbacks=callbacks, batch_size=100,validation_data=(X_validation,y_validation))
